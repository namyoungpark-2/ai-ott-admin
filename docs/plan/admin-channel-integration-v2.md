...
- [x] 채널 관리 라우트 추가
- [x] 채널 목록 테이블 (handle, 이름, 구독자수, 상태)
- [x] `isOfficial === true` 뱃지 표시
- [x] 채널 정지/활성화 토글 버튼
- [x] 정지 시 확인 다이얼로그
- [x] 사이드바 네비게이션에 "채널 관리" 메뉴 추가
...
- [x] 콘텐츠 목록 테이블에 "채널" 컬럼 추가 (`channelName` 표시)
- [x] 콘텐츠 상세 페이지에 채널 정보 표시
- [x] 채널명 클릭 → `/admin/channels` 페이지로 이동
